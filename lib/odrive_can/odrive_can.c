...
static void log_msg(const can_message_t* msg, const char* str) {
...
bool odrive_send_command(uint8_t axis_id, uint8_t cmd_id, const void* buf, uint8_t len) 
{
    can_message_t msg = {};
    msg.identifier = ODRIVE_MSG_ID(axis_id, cmd_id);
    msg.data_length_code = len;
    memcpy(msg.data, buf, len);
    return can_transmit(&msg, 0) == ESP_OK;
}
...
esp_err_t odrive_receive_updates(ODriveAxis* axes, uint8_t len) {
    ...
    esp_err_t error;
    can_message_t msg;
    while ((error = can_receive(&msg, 0)) == ESP_OK) {
        if (msg.flags) {
